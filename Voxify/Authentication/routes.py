...
def voter_login():
    ...
    if request.method == "POST":
        ...
        print(f"Login attempt - Student ID: {username}")         
        ...
        if user:
            print(f"User found: {user['student_id']}, Role: {user['role']}")         
...
def forgot_password():
    """Step 1: User submits their Student ID / username to receive a reset OTP."""
    role_type = request.args.get('role', 'voter')                      

    if request.method == "POST":
        identifier = request.form.get("identifier", "").strip()
        role_type = request.form.get("role_type", "voter")

        if not identifier:
            flash("Please enter your Student ID.", "error")
            return render_template("forgot_password.html", role_type=role_type)
        ...
        if not user:
            flash("If that account exists, an OTP has been sent to its registered email.", "info")
            return render_template("forgot_password.html", role_type=role_type)

        email = user.get("email")
        if not email or "@" not in str(email):
            flash("No valid email is associated with this account. Please contact the administrator.", "error")
            return render_template("forgot_password.html", role_type=role_type)

                             
        clear_otp_from_session("forgot_password")
        otp = generate_otp()
        store_otp_in_session(otp, "forgot_password", {
            "user_id": user["id"],
            "email": email,
            "role_type": role_type,
        })
...
def verify_forgot_otp():
    ...
    user_data = session.get("user_data_forgot_password")
    if not user_data:
        flash("Session expired. Please start the forgot-password process again.", "warning")
        return redirect(url_for("auth.forgot_password"))

    otp_key = "otp_forgot_password"
    otp_expiry = session.get(otp_key, {}).get("expiry")

    if request.method == "POST":
        otp = request.form.get("otp", "").strip()
        if not otp:
            flash("Please enter the OTP.", "error")
            return render_template("verify_forgot_otp.html", otp_expiry=otp_expiry)

        success, message = verify_otp_from_session(otp, "forgot_password")
        if success:
                                                                              
            session["forgot_password_verified"] = True
            session["forgot_password_user_id"] = user_data["user_id"]
            session["forgot_password_role_type"] = user_data.get("role_type", "voter")
            session.modified = True
...
def reset_password():
    ...
    if not session.get("forgot_password_verified"):
        flash("Unauthorized. Please verify your OTP first.", "error")
        return redirect(url_for("auth.forgot_password"))

    user_id = session.get("forgot_password_user_id")
    role_type = session.get("forgot_password_role_type", "voter")
    ...
    if request.method == "POST":
        ...
        for key in ["forgot_password_verified", "forgot_password_user_id",
                    "forgot_password_role_type", "user_data_forgot_password"]:
            session.pop(key, None)
        session.modified = True
...
def resend_forgot_otp():
    ...
    email = user_data.get("email")
    if not email or "@" not in str(email):
        flash("No valid email on file. Please contact the administrator.", "error")
        return redirect(url_for("auth.verify_forgot_otp"))

    otp = generate_otp()
    store_otp_in_session(otp, "forgot_password", user_data)
    send_otp_email(email, otp)
    fallback_otp = session.pop("_otp_fallback", None)
    session.modified = True
